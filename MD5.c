...
#include <string.h> /* std memcpy and memset */
#include "MD5.h"
...
static void MD5Transform(unsigned int*, const unsigned char*);
...
MD5Update(MD5_CTX* context, const unsigned char* input, unsigned int inputLen)
{
  ...
  if (inputLen >= partLen) {
    memcpy((unsigned char*)&context->buffer[index], input, partLen);
 ...
 */
static void
Encode(unsigned char* output, const unsigned int* input, unsigned int len)
{
  unsigned int i, j;
